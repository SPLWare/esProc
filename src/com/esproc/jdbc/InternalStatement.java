...
public class InternalStatement implements java.sql.Statement {
	...
	public Object executeJDBC(String sql, ArrayList<?> parameters,
		...
		try {
			...
			if (sqlType == JDBCUtil.TYPE_NONE)
				return null;
			if (isUpdate
					&& (sqlType != JDBCUtil.TYPE_SQL && sqlType != JDBCUtil.TYPE_SIMPLE_SQL)) {
			...
			if (config != null
				 ...
				 * dfx and the table sequence or cursor is returned. dfx only
			...
			if (con.isOnlyServer()) {
				...
				if (sqlType == JDBCUtil.TYPE_CALL
						|| sqlType == JDBCUtil.TYPE_CALLS
						|| sqlType == JDBCUtil.TYPE_DFX) {
			...
			if (isRemote) {
				...
				Sequence args;
				if (sqlType == JDBCUtil.TYPE_CALLS) {
					args = JDBCUtil
	...
	/**
	 * Execute the gateway dfx file. The gateway is configured in
	 * raqsoftConfig.xml.
	 * 
	 * @param sql
	 *            The SQL string
	...
	private Object executeGateway(String sql, ArrayList<Object> parameters,
		...
		/*
		 * After the gateway is configured, the statements are parsed by dfx and
		 * the table sequence or cursor is returned. dfx only has parameters sql
		...
		if (pl == null || pl.count() != 2) {
			throw new SQLException(
					"The parameters of the gateway dfx file should be sql and arguments.");
		}
