...
import com.scudata.ide.dfx.MenuDfx;
...
public class PanelValueBar extends JPanel {
	...
	/** 显示固定格值 */
	private final String PIN_ON = dfxMM.getMessage("panelvaluebar.pin1");
	/** 显示焦点格值 */
	private final String PIN_OFF = dfxMM.getMessage("panelvaluebar.pin2");
	...
	public PanelValueBar() {
		...
		jBCell.setToolTipText(dfxMM.getMessage("panelvaluebar.cell")); // 字段内容
		...
		jBDrawChart
				.setIcon(GM.getImageIcon(GC.IMAGES_PATH + "m_drawchart.gif"));
		jBDrawChart.setToolTipText(dfxMM.getMessage("panelvaluebar.chart")); // 图形绘制
	...
	/** 复制数据 */
	private final String TIP_COPY_CONTENT = dfxMM
			.getMessage("panelvaluebar.copy");
	/** 复制列名 */
	private final String TIP_COPY_COLNAMES = dfxMM
			.getMessage("panelvaluebar.copycolnames");
	 ...
	 */
	public void refresh() {
		JTableValue table = GVDfx.panelValue.tableValue;
		...
		jBCell.setEnabled(table.getSelectedRow() > -1
		...
		if (GV.appMenu instanceof MenuDfx) {
			((MenuDfx) GV.appMenu).setMenuEnabled(GCDfx.iDRAW_CHART,
					table.canDrawChart());
		}
	}
