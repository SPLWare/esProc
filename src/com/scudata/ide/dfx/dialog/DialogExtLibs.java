...
import com.scudata.ide.dfx.base.TableExtLibs;
...
public class DialogExtLibs extends JDialog implements ActionListener {
	...
	public DialogExtLibs(RaqsoftConfig config, Frame frame, String extLibsPath,
			List<String> extLibs) {
		super(frame, IdeDfxMessage.get().getMessage("dialogselectnames.title"),
				true);
	...
	private synchronized void setExtLibsPath(boolean showException) {
		...
		extLibsPath = GMDfx.getAbsolutePath(extLibsPath);
	...
	private void init() {
		...
		JLabel labelTips = new JLabel(IdeDfxMessage.get().getMessage(
