...
public class ContentPanel extends JPanel implements InputMethodListener,
	...
	public ContentPanel(CellSet cellSet, int startRow, int endRow,
		...
		this.jsp = jsp;
		if (jsp instanceof DfxControl) {
			control = (DfxControl) jsp;
		}
	...
	private void drawText(String text, Graphics g, int row, int col, int x,
		...
		try {
			if (control.getStepPosition() != null && !control.dfx.isAutoCalc()) {
				CellLocation cp = control.getStepPosition();
				int rr = cp.getRow();
				int cc = cp.getCol();
				if (rr == row && cc == col) {
	...
	public void initEditor(byte mode) {
		...
		try {
			...
			String text = ControlUtils.getCellText(control.dfx, cl.getRow(),
					cl.getCol(), true);
			ca = text.length();
		} catch (Throwable t) {
		}
	...
	public void initEditor(int caretPosition, byte mode) {
		...
		if (control.dfx.getCurrentPrivilege() != PgmCellSet.PRIVILEGE_FULL) {
			if (editor != null && editor.isVisible()) {
				editor.setVisible(false);
			}
			return;
