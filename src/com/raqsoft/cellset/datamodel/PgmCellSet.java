...
import com.raqsoft.thread.CursorLooper;
...
public class PgmCellSet extends CellSet {
	...
	private static class CursorForCmdCode extends ForCmdCode {
		...
		public CursorForCmdCode(int r, int c, int endRow, ICursor cursor, int count, 
				Expression gexp, Context ctx) {
			super(r, c, endRow);
			this.cursor = cursor;
			this.count = count;
			this.gexp = gexp;
	...
	public INormalCell getCurrent() {
		return curLct == null ? null : getNormalCell(curLct.getRow(), curLct.getCol());
	}
		...
		if (args != null) {
			...
			for (int i = 0, pcount = args.length; i < pcount; ++i) {
				...
				if (paramCol < colCount) {
					...
					//    MessageManager mm = EngineMessage.get();
					//    throw new RQException("call" + mm.getMessage("function.paramCountNotMatch"));
					//}
					//paramRow++;
					//paramCol = 1;
				}
	...
	private void runContinueCmd(Command command) {
		...
		for (int i = 0, size = stack.size(); i < size; ++i) {
			...
			if (cmd.type == Command.FOR) {
				if (forLct == null ||
					(forLct.getRow() == cmd.row &&
					forLct.getCol() == cmd.col)) {
					index = i;
					break;
				}
			}
	...
	// 创建用于执行fork命令的网格
	private PgmCellSet newForkPgmCellSet(int row, int col, int endRow, Context ctx, boolean isLocal) {
		int rowCount = getRowCount();
		int colCount = getColCount();
	...
	private void executeFork(int row, int col, int endRow) {
		...
		if (lct == null || lct.getRow() > endRow) return;
	...
	private void runForkxCmd(IParam param, int row, int col, int endRow, Context ctx) {
		...
		if (nextRow <= getRowCount()) {
			...
			if (command != null && command.getType() == Command.REDUCE) {
				...
				PgmCellSet reduce = newForkPgmCellSet(nextRow, col, reduceEndRow, ctx, false);
		...
		if (js != null) caller.setJobSpaceId(js.getID());
	...
	private void runForkCmd(IParam param, int row, int col, int endRow, Context ctx) {
		...
		if (param == null) {
			...
			//MessageManager mm = EngineMessage.get();
			//throw new RQException("fork" + mm.getMessage("function.missingParam"));
			...
			if (val instanceof MultipathCursors) {
				ICursor []cursors = ((MultipathCursors)val).getParallelCursors();
		...
		if (dbObj instanceof DBObject) {
			...
			if (command.isQuery()) {
				if (opt == null || opt.indexOf('1') == -1) {
					val = ((DBObject)dbObj).query(sql, paramVals, types, opt, ctx);
				} else {
	...
	private CellLocation runNext2() {
		...
		try {
			...
			if (command == null) {
				...
				case Command.RESULT:
					//MessageManager mm = EngineMessage.get();
					//throw new RQException(mm.getMessage("engine.unknownRet"));
	 ...
	 * @return Object 子函数返回值
	 */
	/*public Object executeFunc_nr(int row, int col, Object []args) {
		if (row < 1 || row > getRowCount() || col < 1 || col > getColCount()) {
			MessageManager mm = EngineMessage.get();
			throw new RQException(mm.getMessage("engine.callNeedSub"));
		}
		...
		try {
			...
			for (; curLct != null;) {
				...
				int curCol = curLct.getCol();
				cell = getPgmNormalCell(curRow, curCol);
				cmd = cell.getCommand();
				if (cmd == null) {
					runNext2();
				} else if (cmd.getType() == Command.RETURN) {
	...
	public Object nextResult() {
		...
		if (resultCurrent < 1) resultCurrent = 1; // 首次取
