...
import com.raqsoft.expression.ElementRef;
...
import com.raqsoft.expression.VarParam;
...
public class CellEditingListener implements KeyListener {
	 ...
	 *            内容面板
	 */
	public CellEditingListener(DfxControl control, ContentPanel panel) {
	...
	private void matchKeyWord(JTextComponent jtext) {
		if (GVDfx.matchWindow != null) {
			...
			int dot = GVDfx.matchWindow.getDot();
			if (caret < dot || dot > text.length()) {
				stopMatch();
				return;
			}
			boolean isPeriod = GVDfx.matchWindow.isPeriod();
			...
			name = name.trim();
			if (GVDfx.matchWindow != null) {
				boolean isSearched = GVDfx.matchWindow.searchName(name);
				if (!isSearched) {
					stopMatch();
				}
			}
	...
	public void keyPressed(KeyEvent e) {
		int key = e.getKeyCode();
		boolean isMatching = GVDfx.matchWindow != null
				&& GVDfx.matchWindow.isVisible();
		switch (key) {
				...
				if (nextCol > 0) {
					...
					if (control.getActiveCell() != null) {
						...
						if (curRow == ics.getRowCount()) {
							control.getContentPanel().submitEditor();
							DfxEditor editor = ControlUtils
									.extractDfxEditor(control);
							control.getContentPanel().revalidate();
							editor.appendRows(1);
						}
					}
		...
		case KeyEvent.VK_TAB:
			if (e.isShiftDown()) {
				...
				if (curCol == ics.getColCount()) {
					control.getContentPanel().submitEditor();
					ControlUtils.extractDfxEditor(control).appendCols(1);
				}
		...
		case KeyEvent.VK_UP:
			if (isMatching) {
				if (GVDfx.matchWindow != null) {
					GVDfx.matchWindow.selectBefore();
				}
			}
			if (e.isAltDown()) {
				return;
	...
	private boolean startMatch(final JTextComponent jtext) {
		...
		try {
			...
			Context ctx = control.dfx.getContext();
			Object val = null;
			if (isPeriod) {
			...
			if (preStr.indexOf(".") > 0 || preStr.indexOf("(") > 0) {
				Expression exp = new Expression(control.dfx, ctx, preStr);
				...
				if (cl != null) {
					PgmNormalCell cell = control.dfx.getPgmNormalCell(
							cl.getRow(), cl.getCol());
	...
	private boolean showMatchWindow(final JTextComponent jtext, final int p,
		...
		if (ds != null) {
			...
			GVDfx.matchWindow = new JWindowNames(fieldNames, p, isPeriod) {
			...
			int w = 150;
			FontMetrics fmWindow = GVDfx.matchWindow
					.getFontMetrics(GVDfx.matchWindow.getFont());
			...
			GVDfx.matchWindow.setBounds(x, y, w, h);
			GVDfx.matchWindow.setVisible(true);
			jtext.requestFocus();
			return true;
		}
		return false;
