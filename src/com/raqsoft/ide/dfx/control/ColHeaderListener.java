...
public class ColHeaderListener implements MouseMotionListener, MouseListener,
	 ...
	 *            网格控件
	 */
	public ColHeaderListener(DfxControl control) {
		this(control, true);
	}
	 ...
	 *            是否可以编辑
	 */
	public ColHeaderListener(DfxControl control, boolean editable) {
	...
	public void mousePressed(MouseEvent e) {
		...
		if (e.getButton() == MouseEvent.BUTTON1 || !colIsSelected) {
			...
			if (control.status != GCDfx.STATUS_CELLRESIZE
				...
				if (firstRow < 1 || firstRow > control.dfx.getRowCount())
					firstRow = 1;
				...
				if (e.isShiftDown() && this.startSelectedCol > 0) {
					...
					control.addSelectedArea(
							new Area(1, start, control.dfx.getRowCount(), end),
							true);
				} else {
					this.startSelectedCol = col;
					control.addSelectedCol(new Integer(col));
					control.addSelectedArea(
							new Area(1, col, control.dfx.getRowCount(), col),
							false);
				}
				control.repaint();
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (control.status == GCDfx.STATUS_CELLRESIZE) {
			if (resizeStartCol > 0) {
				...
				IColCell cc = control.dfx.getColCell(col);
				if (cc != null) {
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getButton() == MouseEvent.BUTTON1 && e.getClickCount() == 2) {
			...
			if (x >= control.cellX[col] + control.cellW[col] - 2
				...
				ControlUtils.extractDfxEditor(control).selectedCols = cols;
				ControlUtils.extractDfxEditor(control).adjustColWidth();
			}
		}
	}
	...
	public void mouseDragged(MouseEvent e) {
		...
		if (control.status == GCDfx.STATUS_NORMAL) {
			...
			control.addSelectedArea(
					new Area(1, start, control.dfx.getRowCount(), end), true);
	...
	public void keyPressed(KeyEvent e) {
		...
		case KeyEvent.VK_RIGHT:
			...
			if (end > control.dfx.getColCount()) {
				end = (int) control.dfx.getColCount();
			}
			if (start < 1) {
				start = 1;
			}
			...
			control.addSelectedArea(
					new Area(1, (int) start, control.dfx.getRowCount(),
							(int) end), true);
			if (ControlUtils.scrollToVisible(control.getColumnHeader(),
					control, 0, (int) end)) {
				Point p1 = control.getColumnHeader().getViewPosition();
