...
public abstract class DfxControl extends ControlBase {
	...
	public Area toUpCell() {
		...
		CellSetParser parser = new CellSetParser(this.dfx);
	...
	public Area toLeftCell() {
		...
		CellSetParser parser = new CellSetParser(dfx);
	...
	private void updateCoords() {
		...
		for (int i = 1; i < cols; i++) {
			...
			if (!parser.isColVisible(i)) {
				...
				cellW[i] = (int) dfx.getColCell(i).getWidth();
			}
		}
	}
	...
	public void selectAll() {
		...
		int rows = dfx.getRowCount();
		int cols = (int) dfx.getColCount();
	...
	public void insertColumn(int col, int count) {
		if (col > dfx.getColCount() || col < 0) {
			col = dfx.getColCount();
		}
		dfx.insertCol(col, count);
		resetControlWidth();
	}
	...
	public List<NormalCell> removeColumn(int col, int count) {
		...
		if (col <= dfx.getColCount() && col > 0) {
			adjustCells = dfx.removeCol(col, count);
	...
	public void insertRow(int row, int count) {
		if (row > dfx.getRowCount() || row < 0) {
			row = dfx.getRowCount();
		}
		dfx.insertRow(row, count);
		resetControlHeight();
	}
	...
	public List<NormalCell> removeRow(int row, int count) {
		List<NormalCell> adjustCells = null;
		if (row <= dfx.getRowCount() && row > 0) {
			adjustCells = dfx.removeRow(row, count);
			if (getActiveCell() != null) {
	...
	private boolean moveRect(CellRect srcRect, CellRect tarRect,
		...
		Vector<IAtomicCmd> cmds = GMDfx.getMoveRectCmd(
				ControlUtils.extractDfxEditor(this), srcRect, tarRect);
		if (cmds == null) {
			return false;
		}
		ControlUtils.extractDfxEditor(this).executeCmd(cmds);
		if (scrollToTarget) {
	...
	private void connectRowUpTo(int connectRow, int upCol) {
		...
		if (cmds != null && !cmds.isEmpty()) {
			AtomicDfx cmd = new AtomicDfx(this);
