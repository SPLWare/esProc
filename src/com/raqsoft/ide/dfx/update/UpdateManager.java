package com.raqsoft.ide.dfx.update;
...
import com.raqsoft.ide.common.dialog.DialogCheckUpdate;
...
public class UpdateManager {
	...
	public static void checkUpdate(final boolean isAuto) throws Exception {
		...
		if (!serverFile.exists())
			return;
		HashMap<String, String> map = readProperties(new FileInputStream(
				serverFile));
			...
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					GM.showException(
							e,
							true,
							GMDfx.getLogoImage(true),
							IdeCommonMessage.get().getMessage(
									"updatemanager.premessage"), GV.appFrame);
				}
			});
			return;
		}
		if (serverVersion != null && localVersion != null
				&& serverVersion.compareTo(localVersion) > 0) { // 有新版本
	 ...
	 * @throws IOException
	 */
	private static HashMap<String, String> readProperties(InputStream is)
			throws IOException {
