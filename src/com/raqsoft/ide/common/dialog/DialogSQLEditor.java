...
public class DialogSQLEditor extends JDialog {
	...
	void jButtonFL_actionPerformed(ActionEvent e) {
		...
		String sSql = jTextPaneSql.getText();
		String tmpFrom = addTableQuote(fromListRight.totalItems());
		if (!com.raqsoft.common.StringUtils.isValidString(tmpFrom)) {
			sSql = "";
		} else {
			sSql = SQLParser.modify(sSql, SQLParser.KEY_FROM, tmpFrom);
		}
	...
	void jBOK_actionPerformed(ActionEvent e) {
		...
		if (!isCopyMode) {
			...
			if (!com.raqsoft.common.StringUtils.isValidString(ls_sql)) {
				return;
			}
			ls_sql = ls_sql.trim();
	...
	private String generateSql(int tabIndex) {
		...
		case TAB_SELECT:
			...
			if (!com.raqsoft.common.StringUtils.isValidString(sTmp)) {
				sTmp = "*";
			}
