...
import com.raqsoft.expression.mfn.sequence.Avg;
import com.raqsoft.expression.mfn.sequence.Count;
import com.raqsoft.expression.mfn.sequence.Max;
import com.raqsoft.expression.mfn.sequence.Min;
import com.raqsoft.expression.mfn.sequence.New;
import com.raqsoft.expression.mfn.sequence.Sum;
...
public class JoinCursor extends ICursor {
	...
	private void init() {
		...
		for (int i = 0, len = exps.length; i < len; i++) {
			...
			if (home instanceof UnknownSymbol) {
				...
				} else if (home instanceof com.raqsoft.expression.fn.gather.Top) {
					IParam fieldParam = ((com.raqsoft.expression.fn.gather.Top) exp.getHome()).getParam();
					if (fieldParam != null) {
	...
	protected Sequence getData(int n) {
		...
		int keyCount = this.keyCount;
		int csFieldsCount = this.csFieldsCount;
		int len = (isNew || isNews) ? ds.getFieldCount() : ds.getFieldCount() - csFieldsCount;//derive时要减去A/cs的字段数
						...
						if (isNew) {
							...
							for (int i = 0; i < len; i++) {
								...
								if (idx < 0)
									...
									record.setNormalFieldValue(i + csFieldsCount, objs[idx]);
							}
						}
			...
			while (true) {
				...
				if (cmp == 0) {
					...
					if (0 != Variant.compareArrays(keys2, keys1)) {
						...
						Record record = newTable.newLast();
						calcExps(record, tempTable, r, len);
	 ...
	 * @param len
	 */
	private void calcExps(Record record, Table tempTable, Record r, int len) {
	...
	private static Node parseNode(Expression exp, Context ctx) {
		...
		if (home instanceof Moves) {
			...
			if (fname.equals("sum")) {
				...
				node = new com.raqsoft.expression.mfn.sequence.Top();
				((Function) node).setParameter(null, ctx, ((Function)exp.getHome()).getParamString());
			}
		}
		return node;
