...
import com.raqsoft.chart.element.IMapAxis;
...
//			集算器图元会用到报表统计图，而统计图的配置文件为color.properties,且与集算器格式不同
			String relativePath = com.raqsoft.ide.common.GC.PATH_CONFIG
					+ name;
			File f = new File(
					com.raqsoft.ide.common.GM.getAbsolutePath(relativePath));
			if (f.exists()) {
