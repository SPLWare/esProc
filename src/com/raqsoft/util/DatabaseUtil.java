		...
		try {
			...
			if (paramCount > 0) {
				...
				for (int paramIndex = 0; paramIndex < paramCount; paramIndex++) {
					...
					if (types == null || types.length <= paramIndex) {
						argTypes[paramIndex] = com.raqsoft.common.Types.DT_DEFAULT;
					} else {
			...
			if (opt != null && opt.indexOf("t") > -1) {
				...
				if (tableName == null || tableName.trim().length() < 1) {
					tableName = com.raqsoft.common.SQLParser.getClause(sql, com.raqsoft.common.SQLParser.KEY_FROM);
		...
		try {
			...
			if (paramCount > 0) {
				...
				for (int i = 0; i < paramCount; i++) {
					...
					if (mode == DatabaseUtil.PROC_MODE_OUT || mode == DatabaseUtil.PROC_MODE_INOUT) {
						hasOutParam = true;
						if (types[i] != com.raqsoft.common.Types.DT_CURSOR) {
							outCount++;
						} else {
							outCursor++;
						}
			...
			for (int iArg = 0; iArg < paramCount; iArg++) {
				...
				if (mode == DatabaseUtil.PROC_MODE_IN) {
					...
					paramIndex++;
					if (com.raqsoft.common.Types.DT_CURSOR == types[iArg]) {
						if (dbType == DBTypes.ORACLE) {
							try {
		...
		try {
			...
			while (true) {
							...
							for (int ci = 0; ci < colSize; ci++) {
								...
								for (int fi = 0; fi < fsize; fi++) {
									if (colname.equalsIgnoreCase(fields[fi])) {
										...
										byte rqType = com.raqsoft.common.Types.getTypeBySQLType(sqlType);
										tColTypes[fi] = rqType;
										break;
									}
								}
				...
				try {
					...
					com.raqsoft.common.Logger.debug("update error:", e);
					if (dbs.getErrorMode()) {
