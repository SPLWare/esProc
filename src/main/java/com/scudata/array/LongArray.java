...
public class LongArray implements NumberArray {
	...
	private long []datas; // 第0个元素表示是否是临时数组
	private boolean []signs; // 表示相应位置的元素是否是null，有null成员时才产生
	private int size;
	
	public LongArray() {
		datas = new long[DEFAULT_LEN];
	}

	public LongArray(int initialCapacity) {
		datas = new long[++initialCapacity];
	}
	...
	private static int compare(long n1, Object o2) {
		if (o2 instanceof Integer || o2 instanceof Long) {
			...
			throw new RQException(mm.getMessage("Variant2.illCompare", n1, o2,
					mm.getMessage("DataType.Long"), Variant.getDataType(o2)));
		}
	}
	...
	public IArray dup() {
		...
		return new LongArray(newDatas, newSigns, size);
	}
	...
	public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
		...
		size = in.readInt();
		int len = size + 1;
		long []datas = this.datas = new long[len];
	...
	public void addAll(Object []array) {
		...
		for (int i = 0; i < size2; ++i) {
			if (array[i] != null) {
				datas[++size] = ((Number)array[i]).longValue();
			} else {
				if (signs == null) {
					this.signs = signs = new boolean[datas.length];
				}
	...
	public void insertAll(int pos, IArray array) {
		if (array instanceof LongArray) {
			...
			System.arraycopy(longArray.datas, 1, datas, pos, numNew);
			if (longArray.signs == null) {
				if (signs != null) {
					boolean []signs = this.signs;
				...
				System.arraycopy(longArray.signs, 1, signs, pos, numNew);
			}
	...
	public void push(IArray array, int index) {
		if (array instanceof LongArray) {
			if (array.isNull(index)) {
				...
				datas[++size] = ((LongArray)array).getLong(index);
			}
		} else {
	...
	public void set(int curIndex, IArray array, int index) {
		if (array.isNull(index)) {
			...
			datas[curIndex] = ((LongArray)array).getLong(index);
		} else {
	...
	public IArray get(int []indexArray, int start, int end, boolean doCheck) {
		...
		if (doCheck) {
			...
			if (signs == null) {
				long []resultDatas = new long[len + 1];
			...
			if (isSorted) {
				for (int i = 1; i <= resultSize; ++i) {
					if (result.isTrue(i)) {
						...
						} else if (binarySearch(array.getLong(i)) < 1) {
							result.set(i, false);
						}
					}
				}
				...
				for (int i = 1; i <= resultSize; ++i) {
					if (result.isTrue(i)) {
						...
						} else if (!contains(array.getLong(i))) {
							result.set(i, false);
						}
					}
				}
	...
	public IArray negate() {
		...
		if (isTemporary()) {
			...
			IArray  result = new LongArray(newDatas, newSigns, size);
			result.setTemporary(true);
			return result;
		}
	}
		...
		for (int i = 1; i <= size; ++i) {
			...
			if (v == null) {
				...
				if (s1 == null || !s1[i]) {
					...
					resultDatas[i] = -((Number)v).longValue();
				}
			} else {
				MessageManager mm = EngineMessage.get();
				throw new RQException(getDataType() + mm.getMessage("Variant2.with") +
	...
	public int compareTo(int curIndex, IArray array, int index) {
		if (isNull(curIndex)) {
			...
			return compare(datas[curIndex], array.getLong(index));
		} else if (array instanceof DoubleArray) {
	...
	public IArray memberAdd(int curIndex, IArray array, int index) {
		if (array.isNull(index)) {
			...
			if (signs == null || !signs[curIndex]) {
				datas[curIndex] += array.getLong(index);
			} else {
				datas[curIndex] = array.getLong(index);
			...
			return memberAdd(curIndex, ((DoubleArray)array).getDouble(index));
		} else {
			Object obj = array.get(index);
			if (obj instanceof Integer || obj instanceof Long) {
				if (signs == null || !signs[curIndex]) {
					datas[curIndex] += ((Number)obj).longValue();
				} else {
					datas[curIndex] = ((Number)obj).longValue();
					signs[curIndex] = false;
				}
				...
				return memberAdd(curIndex, ((Number)obj).doubleValue());
	...
	public int hashCode(int index) {
		if (signs == null || !signs[index]) {
			return Long.hashCode(datas[index]);
		} else {
			return 0;
		}
	}
			...
			if (isIncre) {
				...
				if (isSorted) { // 源序列有序
					...
					for (int t = 1; t <= subLen; ++t) {
						if (array.isNull(t)) {
							...
							pos = binarySearch(array.getLong(t), pos, len);
						}
					...
					for (int t = 1; t <= subLen; ++t) {
						if (array.isNull(t)) {
							...
							pos = firstIndexOf(array.getLong(t), pos);
						}
				...
				if (isSorted) {
					...
					while (candidate <= maxCandidate) {
						int result = compareTo(candidate, array, 1);

						if (result < 0) {
							candidate++;
						} else if (result == 0) {
							for (int i = 2, j = candidate + 1; i <= subLen; ++i, ++j) {
								if (!isEquals(j, array, i)) {
									candidate++;
									continue Next;
								}
							}
				...
				if (isSorted) { // 源序列有序
					for (int t = 1; t <= subLen; ++t) {
						if (array.isNull(t)) {
							...
							pos = binarySearch(array.getLong(t));
						}
					...
					for (int t = 1; t <= subLen; ++t) {
						if (array.isNull(t)) {
							...
							pos = firstIndexOf(array.getLong(t), 1);
						}
