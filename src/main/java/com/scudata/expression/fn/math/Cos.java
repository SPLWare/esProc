...
public class Cos extends Function {
	...
	public void checkValidity() {
		if (param == null) {
			...
			throw new RQException("cos" + mm.getMessage("function.missingParam"));
		} else if (!param.isLeaf()) {
			MessageManager mm = EngineMessage.get();
			throw new RQException("cos" + mm.getMessage("function.invalidParam"));
		}
	}
	...
	public Object calculate(Context ctx) {
		...
		if (obj instanceof Number) {
			...
			MessageManager mm = EngineMessage.get();
			throw new RQException("cos" + mm.getMessage("function.paramTypeError"));
		}
	}
	...
	public IArray calculateAll(Context ctx) {
		...
		if (array instanceof ConstArray) {
			...
			if (obj instanceof Number) {
				double v = Math.cos(((Number)obj).doubleValue());
		...
		if (array instanceof NumberArray) {
			...
			for (int i = 1; i <= size; ++i) {
				if (numberArray.isNull(i)) {
					...
					double v = Math.cos(numberArray.getDouble(i));
					result.push(v);
				}
			}
		} else {
