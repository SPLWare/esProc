...
public class Derive extends PhyTableFunction {
	...
	public static Object derive(IPhyTable table, ICursor cursor, Object obj, Expression filter, Expression []exps,
		...
		if (cursor instanceof MultipathCursors) {
			return JoinCursor2.makeMultiJoinCursor(table, exps, names, (MultipathCursors)cursor, null, filter, fkNames, codes, opts, 2, null, ctx);
		}
		ICursor cs = new JoinCursor2(table, exps, names, cursor, null, filter, fkNames, codes, opts, 0, null, ctx);
		if (obj instanceof Sequence) {
			return cs.fetch();
		} else {
			return cs;
