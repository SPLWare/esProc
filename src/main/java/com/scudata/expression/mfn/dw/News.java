...
public class News extends PhyTableFunction {
	public Object calculate(Context ctx) {
		...
		if (JoinCursor.isColTable(table)) {
			...
			return news(table, cursor, obj, csNames, filter, exps, names, fkNames, codes, opts, option, ctx);
		}
	}
	...
	public static Object news(IPhyTable table, ICursor cursor, Object obj, String[] csNames, Expression filter, Expression []exps,
			String[] names, String []fkNames, Sequence []codes, String[] opts, String option, Context ctx) {
		if (cursor instanceof MultipathCursors) {
			return JoinCursor2.makeMultiJoinCursor(table, exps, names, (MultipathCursors)cursor, csNames, filter, fkNames, codes, opts, 2, option, ctx);
		}
		ICursor cs = new JoinCursor2(table, exps, names, cursor, csNames, filter, fkNames, codes, opts, 2, option, ctx);
		if (obj instanceof Sequence) {
			return cs.fetch();
		} else {
			return cs;
