...
public class TInv extends Function {
					...
					for (int i = 1; i <= len1; i++) {
						...
						if (o11 instanceof Sequence && o21 instanceof Sequence) {
							...
							for (int j = 1; j<=len2; j++) {
								res.add(tinv0(((Sequence) o11).get(j), ((Sequence) o21).get(j)));
							}
							result.add(res);
						}
					}
	...
	private double tinv0(Object o1, Object o2) {
		double p = o1 instanceof Number ? ((Number) o1).doubleValue() : 0d;
		double f = o2 instanceof Number ? ((Number) o2).doubleValue() : 0d;
		return tinv(p, f);
	}
	
	public static double tinv(double probability, double freedomDegrees) {
        MTDistribution mCSD= new MTDistribution(freedomDegrees);
