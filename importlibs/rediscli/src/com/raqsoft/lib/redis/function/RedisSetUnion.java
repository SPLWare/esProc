...
public class RedisSetUnion extends RedisBase {
	...
	public Object doQuery( Object[] objs){
		...
		if (objs.length != 2){
			throw new RQException("redis sunion param size " + objs.length + " is 2");
		}else{
			Set<String> set = null;
			if (objs[1] instanceof String){
				set = m_jedisTool.sUnion(objs[0].toString(), objs[1].toString());
				...
				set = m_jedisTool.sUnion(objs[0].toString(), ls);
				if (set!=null && set.size()>0){
					List<Object[]> list = new ArrayList<Object[]>();
					for( Iterator<String> it = set.iterator();  it.hasNext(); ){  
						os = new Object[1];
